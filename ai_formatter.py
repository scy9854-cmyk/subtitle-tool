...
3. "reference" — 그 줄이 (괄호로 감싸져 있든 아니든) 성경 구절 참조로 시작하는 경우 (예: "요한복음 3:16", "롬 8:28-30", "삼상 15장", "(약5:15)", "(히7:24-25)", "삼상17:31~32 어떤 사람이 다윗이 한 말을 듣고..."). 참조 뒤에 목사님이 직접 입력/붙여넣은 본문 텍스트가 이어져도 상관없이 reference로 분류하세요 — 그 본문은 오타가 있을 수 있어 실제 성경 원문으로 다시 조회해서 교체할 것이므로 내용 자체는 중요하지 않습니다. book에는 아래 정식 책 이름 목록 중 정확히 일치하는 이름을, chapter에는 장 번호를, startVerse/endVerse에는 절 범위를 넣습니다. 특정 절 없이 장 전체를 가리키면 startVerse=1, endVerse=999로 설정하세요. 절이 하나면 startVerse와 endVerse를 같은 값으로 하세요.
